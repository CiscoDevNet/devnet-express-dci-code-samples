...
# pylint: disable=import-error, consider-using-sys-exit, bare-except, no-else-return, invalid-name, no-else-break, inconsistent-return-statements
...
if not ROOM_NAME or not ACCESS_TOKEN:
    exit(
            "Looks like ROOM_NAME and/or ACCESS_TOKEN variables are empty.\n" +
            "If you are attending a DevNetExpress Event obtain the room name " +
            "from your instructor and populate the ROOM_NAME variable.\n" +
            "To obtain your access token visit https://developer.webex.com " +
            "and assign it to ACCESS_TOKEN variable."
        )
...
if len(ROOM_ID) < 1:
    exit(
            "Provided room name does not exists or has a typo in it.\n" +
...
def json_rpc_payload(cmd):
    """
    MISSION: Obtain payload structure from NX-API Developer
    Sandbox and assign to the payload variable
    ...
    if "jsonrpc" in payload[0] and payload[0]["params"]["cmd"] == cmd:
        ...
        exit(
                "MISSION: Obtain payload structure from NX-API Developer" +
                "Sandbox and assign it to the payload variable"
            )
...
if NX_IP and USERNAME and PASS and CMD:
    ...
    exit(
            "Looks like one of the values required to complete this mission is empty.\n" +
            "Please populate NX_IP, USERNAME, PASS and CMD variables with necessary " +
            "information and re-run the script."
        )
...
if response.ok:
    ...
    exit(
        ...
        "the requested information. The returned error is:\n",
        response.text
    )

# Create an array of mac address found on the target device
mac_list = {
    num: mac["disp_mac_addr"]
    for num, mac in enumerate(mac_table)
    ...
    if user_choice in mac_list:
        ...
        if iface:
            ...
            if response.ok:
                ...
                mission_msg += "\n        Drop due to Interface Down: " + str(
                                                            iface_stats["eth_in_ifdown_drops"]
                                                        )
                mission_msg += "\n    TX Information"
